...
#---------------------------------------------------------------------------------
all: lib/libbuttoncombo.a

dist-bin: all
	@tar --exclude=*~ -cjf libbuttoncombo-$(VERSION).tar.bz2 include lib

dist-src:
	@tar --exclude=*~ -cjf libbuttoncombo-src-$(VERSION).tar.bz2 include source Makefile

dist: dist-src dist-bin

install: dist-bin
	mkdir -p $(DESTDIR)$(DEVKITPRO)/wums
	bzip2 -cd libbuttoncombo-$(VERSION).tar.bz2 | tar -xf - -C $(DESTDIR)$(DEVKITPRO)/wums
...
lib/libbuttoncombo.a :$(SOURCES) $(INCLUDES) | lib release
